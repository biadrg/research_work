...
def train_cycle(
    ...
    for epoch in range(hyperparams["epochs"]):
        ...
        if valid_loss < best_valid_loss:
            ...
            if hyperparams["model_dir"]==None:
                ...
                torch.save(model.state_dict(), f"QuixerTuning/{hyperparams['dataset']}/{hyperparams['parcel_type']}/{hyperparams['model_dir']}_{hyperparams['seed']}_{tuning_set}.pt")
    ...
    if hyperparams["model_dir"]==None:
        ...
        model.load_state_dict(torch.load(f"QuixerTuning/{hyperparams['dataset']}/{hyperparams['parcel_type']}/{hyperparams['model_dir']}_{hyperparams['seed']}_{tuning_set}.pt", weights_only=True))
    if hyperparams["binary"]==True:
    ...
    # Save to CSV
    csv_filename = f"QuixerTuning/{hyperparams['dataset']}/{hyperparams['parcel_type']}/{hyperparams['target']}_{hyperparams['seed']}_{tuning_set}.csv"
